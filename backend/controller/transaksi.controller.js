...
exports.addTransaksi = async (request, response) => {
  ...
  .then(result => {
    ...
    console.log(request.body.detail_transaksi);
    let detailData = request.body.detail_transaksi
    detailData.forEach(val => {
      val.id_transaksi = result.id
    })
    console.log(detailData);
    detailsOfTransaksiModel.bulkCreate(detailData)
      .then(detailResult => {
        response.json({
          message: "Data Berhasil Ditambahkan",
          data: {
